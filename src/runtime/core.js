/** Readable runtime module: shared state, geometry, motion, persistence. */
/**
 * uniRico v0.8.0 readable runtime mirror.
 *
 * This preserves the compact identifiers used by the shipping artifact so
 * behavior can be compared directly. The public names/roles are documented in
...
function $2(){$t=$n=0;for(let r of V)if(r){$n+=r[0]&3;$t+=r[3]||0}U.c.textContent=String($t).padStart(5,'0');U.k.textContent='★ '+String($n).padStart(3,'0')}
...
function $0(){U.l.textContent=String(L+1).padStart(2,'0')+'/'+LEVELS.length;U.s.textContent='S '+$d;U.t.textContent=_c(Y);$2()}
