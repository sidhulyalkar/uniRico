...
const C=document.querySelector('#c'),X=C.getContext('2d'),U={l:document.querySelector('#level'),s:document.querySelector('#shots'),t:document.querySelector('#time'),c:document.querySelector('#score'),k:document.querySelector('#stars'),n:document.querySelector('#next')},W=960,H=600,P=Math.PI,T=P*2,Q='KIJJLJJJOIJHJJGJLNKSRTRRPUUURPUUTSPUVUQV',E=atob('IB9A3XQD8NMAALHRAM7g2MZiAAAg0AAAAACwzQAAAABdyitkwtC5GrkagHDC0N1ZK2Q5iwAAQmBdyqvUuRqAcMLQ3VkrZDmLAABCYKvUuRo=');let R=Math.random,$V=1,L=0,$d=0,$t=0,$n=0,F=0,$h=0,$e=[800,300],B=null,$q=[],$g=[],$r=[],$c=0,$k=0,$a=1,$y=1,$F,J=0,Y=0,$s=0,$m=0,V=[],$W=0,$l=[],_=0,ek=0,et=0,ee=0,mt,mb=0;
...
function $0(){U.l.textContent=String(L+1).padStart(2,'0')+'/'+LEVELS.length;U.s.textContent='S '+$d;U.t.textContent=_c(Y);let i=B?B.i:0,t=O().t[Math.min(i,O().t.length-1)];U.n.textContent='NEXT '+(i+1)+'/'+O().t.length+' · NEED '+t[2]+' BOUNCE'+(t[2]==1?'':'S');$2()}
