...
## v0.20.0 — Reflection Gauntlet + Compression Pipeline

- Expand the deterministic campaign from **40 to 50 levels** without duplicating ten full late-game maps.
- Add Levels 41–50 as a **Reflection Gauntlet** generated by exact 180° transforms of Levels 31–40: launch points, targets, walls, portals, moving geometry, force vectors, gravity/magnet centers, gates, and encoded launch angles are rotated consistently.
- Finish on Level 50, **MIRROR FULL SPECTRUM**, with a six-lock reflected full-spectrum route.
- Extend solution regression coverage from 40/40 to **50/50**, and require every transformed level to preserve its source target count and solve through the authoritative fixed-step physics.
- Extend mechanic-use auditing to all 50 levels so reflected scenery cannot become decorative or bypassable.
- Replace the single Terser-only release path with a deterministic final-byte competition between **Terser + Zopfli** and **Terser + Roadroller 2.1.0 (`-O0`) + Zopfli**. The smaller final ZIP wins.
- Shrink the single-file HTML shell while preserving UTF-8 and the mobile viewport contract.
- Add byte-for-byte rebuild verification to release CI.
- Add exact packed-runtime startup execution after extracting `index.html` from the produced ZIP.
- Current PR candidate: **11,512 / 13,312 bytes**, leaving **1,800 bytes** of headroom while carrying all 50 levels.
- Current candidate SHA-256: `713114a1185abd266ffdd42664217e06170b22673e9afb5eaa7cb3dd9c9a87ff`.
